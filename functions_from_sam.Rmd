...
# Save the 2021 social network plot as a high-resolution PNG
if ("2021" %in% names(social_networks)) {
  # Extract the 2021 social network
  g_2021 <- social_networks[["2021"]]
  ...
  png(
    "C:/Users/balae/OneDrive/Desktop/Claire NBW thesis/data/social_network_2021.png",
    width = 3000, height = 3000, res = 1000  # Increase dimensions to fit the content
  )
  
  # Set up margins to accommodate plot elements
  ...
  plot(
    ...
    vertex.label = NA,                              # Remove vertex labels
    vertex.color = V(g_2021)$color,                 # Color nodes by age class
    main = "Social Network for Year 2021"           # Title
  )
  
  # Close the PNG device
  dev.off()
  
  cat("High-resolution plot for the 2021 social network saved successfully.\n")
} else {
  warning("The 2021 social network does not exist in the dataset.")
}
...
ggplot(glm_data, aes(x = strength)) +
  geom_histogram(
    binwidth = 0.2,                 # Set bin width to 0.2
    fill = "blue",                  # Set fill color
    color = "black",                # Set border color
    alpha = 0.7                     # Set transparency
  ) +
  labs(
    title = "Histogram of HWI Values",
    x = "HWI (Strength)",
    y = "Frequency"
  ) +
  theme_minimal() +
...
glm_data[, dyadic_age := as.factor(dyadic_age)]
glm_data[, dyadic_sex := as.factor(dyadic_sex)]

# Define custom colors for dyadic age
custom_colors_age <- c(
  "Adult_Adult" = "lightblue",
  "Calf_Calf" = "purple",
  "Juvenile_Juvenile" = "darkblue",
  "Adult_Calf" = "pink",
  "Adult_Juvenile" = "blue",
  "Calf_Juvenile" = "violet"
)

# Define custom colors for dyadic sex
custom_colors_sex <- c(
  "FemaleJ_Male" = "red",
  "Male_Male" = "orange",
  "FemaleJ_FemaleJ" = "green"
)

# Extract GLM results and p-values
glm_summary <- summary(glm_model)$coefficients$mean
p_values <- glm_summary[, "Pr(>|z|)"]
significant_terms <- names(p_values[p_values < 0.05])  # Keep only significant results



# Create the first box plot: HWI by Dyadic Age
p1 <- ggplot(glm_data, aes(x = dyadic_age, y = strength, fill = dyadic_age)) +
  geom_boxplot(alpha = 0.7) +
  scale_fill_manual(values = custom_colors_age) +
  labs(y = "HWI (Strength)", x = "Dyadic Age") +
  theme_minimal() +
  theme(
...
# Create the second box plot: HWI by Dyadic Sex
p2 <- ggplot(glm_data, aes(x = dyadic_sex, y = strength, fill = dyadic_sex)) +
  geom_boxplot(alpha = 0.7) +
  scale_fill_manual(values = custom_colors_sex) +
  labs(y = "HWI (Strength)", x = "Dyadic Sex") +
  theme_minimal() +
  theme(
    axis.text.x = element_text(size = 14, angle = 35, hjust = 1),
    axis.text.y = element_text(size = 14),
...
png("C:/Users/balae/OneDrive/Desktop/Claire NBW thesis/data/boxplot_HWI_dyadic_age_sex.png", 
...
cat("Box plots for HWI by dyadic age and dyadic sex saved successfully.\n")
...
# Extract unique combinations of Title, year, and numDaysByYear to create the sampling_period data frame
sampling_period <- unique(Master[, .(Title, year, numDaysByYear)])

# View the result
print(sampling_period)

# Save the resulting data frame to a CSV file (optional)
...
setnames(final_table, old = "Year", new = "year")  # Rename 'Year' in final_table to 'year'

# Check if necessary columns exist in both tables
if (!"Title" %in% names(final_table) | !"year" %in% names(final_table)) {
  stop("final_table must contain 'Title' and 'year' columns.")
}
if (!"Title" %in% names(sampling_period) | !"year" %in% names(sampling_period)) {
  stop("sampling_period must contain 'Title' and 'year' columns.")
}
if (!"numDaysByYear" %in% names(sampling_period)) {
  stop("'numDaysByYear' column is missing in sampling_period.")
}
...
# Ensure Age_Class and Sex are factors
glm_data_filtered[, Age_Class := as.factor(Age_Class)]
glm_data_filtered[, Sex := as.factor(Sex)]

# Create the box plot with separate panels for Sex
p <- ggplot(glm_data_filtered, aes(x = Age_Class, y = Degree, fill = Age_Class)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Exclude outliers
