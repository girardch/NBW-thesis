---
title: "Sam's Functions Markdown"
...
GLM 
-needed to modify the final_results_3 table to add two new columns for dyadic age and sex
-needed to modify the table depicting strength to calculate strength of dyads(?). need to confirm whether the response variable for strength for first glm is measuring strength for the dyads or individual whale 


added to the final_results table so that HWI was new column
```{r}
...
# Calculate HWI (Half-Weight Index)
# HWI = x / (x + 0.5 * (Ya + Yb - 2x))
# x = number of times A and B observed together
# Ya = number of times A observed
# Yb = number of times B observed
...
# Ensure that dyadic_age and dyadic_sex are factors
final_results_4[, dyadic_age := as.factor(dyadic_age)]
final_results_4[, dyadic_sex := as.factor(dyadic_sex)]

# Filter out rows with missing values in required columns
glm_data <- final_results_4[!is.na(HWI) & !is.na(dyadic_age) & !is.na(dyadic_sex)]

# Rename HWI to strength for clarity in the GLM
glm_data[, strength := HWI]

# Build the GLM
glm_model_1 <- glm(strength ~ dyadic_age * dyadic_sex, 
                 data = glm_data, 
                 family = gaussian())
...
saveRDS(glm_model_1, "glm_model_strength.rds")
...
# Ensure that dyadic_age is a factor
final_results_4[, dyadic_age := as.factor(dyadic_age)]

# Filter out rows with missing values in required columns, explicitly exclude NA_NA combinations, and exclude HWI = 0
glm_data <- final_results_4[
  !is.na(HWI) & !is.na(dyadic_age) & !is.na(Year) & 
    dyadic_age != "NA_NA" & HWI != 0
]

# Rename HWI to strength for clarity in the GLM
glm_data[, strength := HWI]

# Adjust HWI values to be within the range (0, 1) for beta regression
epsilon <- 1e-6  # Small adjustment value
glm_data[strength == 1, strength := 1 - epsilon]  # Adjust HWI = 1

# Build the beta regression model with only dyadic_age as a predictor
glm_model <- betareg(strength ~ dyadic_age, 
                     data = glm_data)
...
# Save the GLM model for future use
saveRDS(glm_model, "glm_model_strength_beta_dyadic_age.rds")


```
restrict to pairs where # of opportunities seen is greater than 3
```{r}
library(data.table)

# If not already a data.table, convert
setDT(final_results_4)

# 1. For each pair (A, B), count how many *unique* sampling days
#    in which at least one individual was observed.
final_results_4[, days_any := uniqueN(sampling_day[(A_observed == 1 | B_observed == 1)]),
                by = .(A, B)]

# 2. Filter to keep only dyads with days_any > 3
final_results_4_filtered <- final_results_4[days_any > 3]

# View or use the filtered data
print(final_results_4_filtered)
...
FIRST GLM FINAL
modifying glm to add dyadic_sex
ADDED RANDOM EFFECTS

CALF-CALF not included because dyadic_sex is na-na which has been filtered out
```{r}
# Load necessary libraries
library(data.table)
library(glmmTMB)
...
final_results_4[, dyadic_age := as.factor(dyadic_age)]
final_results_4[, dyadic_sex := as.factor(dyadic_sex)]
final_results_4[, Year := as.factor(Year)]
final_results_4[, A := as.factor(A)]
final_results_4[, B := as.factor(B)]

# Filter out rows with missing values in required columns, explicitly exclude NA_NA combinations, and exclude HWI = 0
glm_data <- final_results_4[
  !is.na(HWI) & !is.na(dyadic_age) & !is.na(dyadic_sex) & !is.na(Year) & 
    dyadic_age != "NA_NA" & dyadic_sex != "NA_NA" & HWI != 0
]

# Rename HWI to edge for clarity in the model
glm_data[, edge := HWI]

# Adjust edge values to be within the range (0, 1) for beta regression
epsilon <- 1e-6  
glm_data[edge == 1, edge := 1 - epsilon]  
glm_data[edge == 0, edge := 0 + epsilon]  

# Fit the mixed-effects beta regression model with multimembership effects
glm_model_5 <- glmmTMB(
  edge ~ dyadic_age + dyadic_sex + (1 | A) + (1 | B) + (1 | Year),
  data = glm_data,
  family = beta_family()
)
...
# Save the model
saveRDS(glm_model_5, "glm_model_5_edge_beta_multimembership.rds")
...
Histogram shows that there are some values where HWI=0 despite filtering fr no 0 in the glm_data, checked with sum(glm_data$HWI == 0, na.rm = TRUE) to confirm there are no zeros
```{r}
...
```{r}
# install.packages("ggeffects")  # Uncomment if not installed
library(ggeffects)
library(ggplot2)
library(gridExtra)

# Predictions for dyadic_age, averaging over random effects
pred_age <- ggpredict(glm_model_5, terms = "dyadic_age", type = "fixed")

# Predictions for dyadic_sex, averaging over random effects
pred_sex <- ggpredict(glm_model_5, terms = "dyadic_sex", type = "fixed")

# Examine the prediction data (optional)
head(pred_age)
head(pred_sex)

# Plot for dyadic_age
p1 <- ggplot(pred_age, aes(x = x, y = predicted)) +
  geom_point(size = 3, color = "darkblue") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                width = 0.2, color = "darkblue") +
  labs(
    x = "Dyadic Age",
    y = "Predicted Edge (Adjusted HWI)",
    title = "Model-Predicted Means by Dyadic Age"
  ) +
  theme_minimal() +
  theme(
...
# Plot for dyadic_sex
p2 <- ggplot(pred_sex, aes(x = x, y = predicted)) +
  ...
  labs(
    x = "Dyadic Sex",
    y = "Predicted Edge (Adjusted HWI)",
    title = "Model-Predicted Means by Dyadic Sex"
  ) +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
...
# Residuals vs Fitted
plot(glm_model$fitted.values, residuals(glm_model), 
...
# Q-Q Plot
qqnorm(residuals(glm_model), main = "Normal Q-Q Plot")
qqline(residuals(glm_model), col = "red")



```

NEW DATA FRAME FOR GLM 2
```{r}
...
```
MERGE NETWORK_METRICS and IS_SEX_AGE TO CREATE FINAL_TABLE
```{r}
...
# Ensure 'Date.Original' is in a valid date format
Master[, Date := as.IDate(Date.Original, format = "%Y-%m-%d")]  # Adjust format if necessary

# Extract year from the Date column (if `year` is not pre-existing or incorrect)
if (!"year" %in% colnames(Master)) {
  Master[, year := year(Date)]  # Extract year if not present
}

# Remove rows with missing values in key columns
Master <- Master[!is.na(Title) & !is.na(year) & !is.na(Date)]

# Debugging: Check unique years
print(unique(Master$year))  # Ensure all years are present
...
```{r}
# Count the unique dates per Title and year in Master
master_count <- Master[, .(masterUniqueDays = uniqueN(Date)), by = .(Title, year)]

# Merge the counts with sampling_period (which has the column numDaysByYear)
comparison <- merge(master_count, sampling_period, by = c("Title", "year"), all = TRUE)

# Create a flag to check if the counts match
comparison[, match := masterUniqueDays == numDaysByYear]
...
if (any(!comparison$match)) {
  cat("There are mismatches between Master and sampling_period:\n")
  print(comparison[match == FALSE])
} else {
...
```
modified to show only Titles that appear more than 3 times in the dataset
REMOVED--> bc it removed all calves
...
SECOND GLM
ADDED RANDOM EFFECTS
Number of social partners ~ age class + additional predictor for # of sampling periods
...
# Step 1: Fit a Poisson GLMM with random effects for Title and Year
glm_model_2 <- glmmTMB(
  Degree ~ Age_Class + numDaysByYear + (1 | Title) + (1 | Year),
  data = glm_data_2,
  family = poisson(link = "log")
)

# Step 2: Check for Overdispersion
dispersion_test <- sum(residuals(glm_model_2, type = "pearson")^2) / df.residual(glm_model_2)
cat("Dispersion test value:", dispersion_test, "\n")

# Step 3: If Overdispersion Exists, Fit a Negative Binomial GLMM
if (dispersion_test > 1.2) {
  cat("Overdispersion detected. Refitting with Negative Binomial Regression...\n")
  glm_model_nb <- glmmTMB(
    Degree ~ Age_Class + numDaysByYear + (1 | Title) + (1 | Year),
    data = glm_data_2,
    family = nbinom2(link = "log")  # Negative Binomial to handle overdispersion
  )
  summary(glm_model_nb)
}

# Step 4: Save the Models
saveRDS(glm_model_2, "glm_social_partners_final.rds")
if (exists("glm_model_nb")) saveRDS(glm_model_nb, "glm_social_partners_nb_final.rds")

# Step 5: Export the dataset for review
write.csv(glm_data_2, "glm_data_2_social_partners_final.csv", row.names = FALSE)
...
p <- ggplot(glm_data_2, aes(x = Age_Class, y = Degree, fill = Age_Class)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Exclude outliers
  scale_fill_manual(
    values = c("Adult" = "lightblue", "Calf" = "purple", "Juvenile" = "darkblue")  # Assign specific colors
  ) +
  scale_y_continuous(
    limits = c(0, 50),  # Shorten the y-axis range
    expand = c(0, 0)    # Remove padding around the y-axis
  ) +
  labs(
    y = "Number of Social Partners",  # Keep the y-axis label
    x = NULL                          # Remove the x-axis label
  ) +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 18),       # Increase size of axis labels
    axis.title = element_text(size = 18),     # Increase size of axis title
...
# Add significance brackets for specific comparisons
p + geom_signif(    # Provide explicit annotation for significance
  y_position = c(45),      # Adjust y-position for bracket above the highest box
  tip_length = 0.01        # Length of the bracket tips
)
...
png("C:/Users/balae/OneDrive/Desktop/Claire NBW thesis/data/boxplot_social_partners.png", 
    width = 1500, height = 800, res = 150)

# Create and save the box plot with significance brackets
plot <- ggplot(glm_data_2, aes(x = Age_Class, y = Degree, fill = Age_Class)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Exclude outliers
  scale_fill_manual(
    values = c("Adult" = "lightblue", "Calf" = "purple", "Juvenile" = "darkblue")  # Assign specific colors
  ) +
  scale_y_continuous(
    limits = c(0, 50),  # Adjust the y-axis range to 50
    expand = c(0, 0)    # Remove padding around the y-axis
  ) +
  labs(
    y = "Number of Social Partners",  # Keep the y-axis label
  ...
  geom_signif(
    comparisons = list(
      c("Adult", "Calf")  # Only significant comparison
    ),
    annotations = c("*"),    # Provide explicit annotation for significance
    y_position = c(48),      # Position the bracket slightly below the y-axis limit
    tip_length = 0.01        # Length of the bracket tips
  )
...
cat("Box plot for social partners saved successfully.\n")
...
modifying the GLM to include sex
```{r}
# Load necessary libraries
library(data.table)
...
glm_data_filtered_2 <- glm_data_2[!(Age_Class == "Juvenile" & Sex == "Male")]
...
visualization
```{r}
# 1. Create a filtered version of your data 
glm_data_filtered_plot <- glm_data_filtered[!(
  Sex == "Male" & Age_Class %in% c("Calf", "Juvenile")
)]

# 2. Plot using the filtered dataset
p <- ggplot(glm_data_filtered_plot, aes(x = Age_Class, y = Degree, fill = Age_Class)) +
  geom_boxplot(alpha = 0.7, outlier.shape = NA) +  # Exclude outliers
  facet_wrap(~Sex) +  # Create separate panels for Male and Female
  scale_fill_manual(
    values = c("Adult" = "lightblue", "Calf" = "purple", "Juvenile" = "darkblue")  # Assign specific colors
  ) +
  scale_y_continuous(
...
building dataset for the final GLM
...
modified dataset to only show individuals that appear more than 3 times
...
FINAL GLM
ADDED RANDOM EFFECTS
prop strength female~juv/calf/male/female
```{r}
# Load necessary libraries
library(data.table)
library(glmmTMB)
...
```{r}
# Install ggeffects if you haven't already
# install.packages("ggeffects")
library(ggeffects)
library(ggplot2)

# Generate predictions for Age_Class using your fitted glm_model
# The bracketed levels [Adult,Calf,Juvenile] ensures these levels appear in the plot
preds <- ggpredict(glm_model, terms = "Age_Class [Adult,Calf,Juvenile]", type = "fixed")

# Plot the predicted proportion of females (on 0-1 scale)
pred_plot <- ggplot(preds, aes(x = x, y = predicted)) +
  geom_point(size = 3, color = "darkblue") +
  geom_errorbar(aes(ymin = conf.low, ymax = conf.high),
                width = 0.2, color = "darkblue") +
  labs(
    x = "Age Class",
    y = "Predicted Proportion Female",
    title = "Predicted Proportion Female by Age Class"
  ) +
  theme_minimal() +
  theme(
    axis.text = element_text(size = 12),
